...
class SubscriptionService:
    def create_subscription(self, db, user_id: int, plan_type: str, stripe_customer_id: Optional[str] = None):
    ...
    def get_user_subscription(self, db, user_id: int):
    ...
    def update_subscription(self, db, subscription_id: int, **kwargs):
    ...
    def cancel_subscription(self, db, subscription_id: int):
    ...
    def get_plan_limits(self, db, plan_type: str):
    ...
    def check_user_limits(self, db, user_id: int, operation: str, **kwargs):
        """Check if a user can perform an operation based on their plan limits"""
        subscription = self.get_user_subscription(db, user_id)
        if not subscription:
            # Free tier limits
            plan_limits = self.get_plan_limits(db, "free")
        else:
            plan_limits = self.get_plan_limits(db, subscription.plan_type)
        
        # Get current usage
        current_usage = self.get_current_usage(db, user_id)
    ...
    def record_usage(self, db, user_id: int, **kwargs):
    ...
    def get_current_usage(self, db, user_id: int):
    ...
    def create_billing_event(self, db, user_id: int, subscription_id: int, event_type: str, 
    ...
    def get_subscription_analytics(self, db, user_id: int):
