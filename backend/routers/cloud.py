...
from backend.models import User, CloudConnection
...
def connect_cloud_provider(provider: str, request: Request, db: Session = Depends(get_db)):
    elevate = request.query_params.get("elevate", "false").lower() == "true"
    current_user = get_current_user_optional(request, db)
    service = get_provider_service(provider)
    if not service or not service.get("start_login"):
        raise HTTPException(status_code=400, detail=f"Unsupported provider: {provider}")
    return service["start_login"](request=request, db=db, user_id=current_user.id, elevate=elevate)
...
def get_files(provider: str, request: Request, folder_id: str = "root", current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
def get_google_photos_files(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return get_files_for_provider('googlephotos', current_user, db, folder_id='root') 